...
module.exports = function(startFn, options) {
  ...
  // MAX_WORKER_MEMORY env var takes precedence over app config.
  if (process.env.MAX_WORKER_MEMORY) {
    options.memoryLimit = +process.env.MAX_WORKER_MEMORY * 1024 * 1024;
  }
  ...
  function monitor() {
    function verifyMemory(worker, pid) {
      pidusage.stat(pid, function(err, stat) {
        ...
        console.log('memory_w=' + memoryUsage);
